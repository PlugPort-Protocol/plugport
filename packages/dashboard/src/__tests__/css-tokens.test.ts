 ...
 * The dark-first tests assert on this rather than pinning exact hex values:
 * a redesign that retunes the palette but keeps :root dark shouldn't fail,
 * while a genuinely inverted :root (light background, dark text) should.
 */
function relativeLuminance(hex: string | undefined): number {
...
describe('T3: CSS Token Correctness', () => {
    ...
    describe(':root is the dark palette (dark-first)', () => {
        it('should use a dark --bg-primary in :root', () => {
            const block = extractBlock(cssContent, ':root');
            const rootProps = extractCustomProperties(block);
            expect(relativeLuminance(rootProps.get('--bg-primary'))).toBeLessThan(0.1);
        });

        it('should use a light --text-primary in :root', () => {
            const block = extractBlock(cssContent, ':root');
            const rootProps = extractCustomProperties(block);
            expect(relativeLuminance(rootProps.get('--text-primary'))).toBeGreaterThan(0.6);
        });
    });
